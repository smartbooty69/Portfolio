...
export default function ContentList({
    ...
    return(
        <div ref={component}>
                ...
                {items.map((item, index) =>(
                    <>
                        {isFilled.keyText(item.data.title) && (
                            <li 
                                key={index} 
                                className="list-item opacity-0" onMouseEnter={()=> onMouseEnter(index)}
            ...
            {/* Hover Element */}
            <div 
                className="hover-reveal pointer-events-none absolute left-0 top-0 -z-10 h-[240px] w-[240px] rounded-3xl bg-cover bg-center oppacity-0 transition-[background] duration-300"
                style={{
